...
int tallyWordsInFile(char *filename, LLNode **wordLists, int maxLen)
{
	...
	// Initializing all the elements in wordLists array to NULL
...
updateWordInTallyList(LLNode **wordListHeads, int maxLen, char *word)
{
	...
	// use strln function to copy the length of the word
	int length = strlen(word);

	// use the llLookupKey fun to check if the word is already in the list associated with its length
	LLNode *foundNode = llLookupKey(wordListHeads[length], word);

	/* TODO: if the word is in the list, then update the tally
	 * in the node and we are done so return success */

	// use if statement to check if the word is in the list
	if (foundNode != NULL)
	{
		// update the tally
		foundNode->value++;
	...
	else
	{
		...
		char *wordNew = strdup(word);
		LLNode *newNode = llNewNode(wordNew, 1);

		// adding the new node at the end of the list using llAppend function
		wordListHeads[length] = llAppend(wordListHeads[length], newNode);
		return 1;
	}
